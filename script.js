class CGPACalculator {
  ...
  initializeUI() {
    ...
    document
      ...
      .addEventListener("click", () => this.addCourseRow());
    document
      .getElementById("addRetake")
      .addEventListener("click", () => this.addRetakeRow());
    document
      .getElementById("calculateButton")
      .addEventListener("click", () => this.calculateFinalCGPA());
    document
      .getElementById("resetButton")
      .addEventListener("click", () => this.resetCalculator());
  ...
  addCourseRow() {
    ...
    const gradeOptions = Object.entries(this.gradeChart)
      .map(
        ([grade, points]) =>
  ...
  calculateFinalCGPA() {
    ...
    document.querySelectorAll(".course-card").forEach((card) => {
      const creditHours =
        parseFloat(card.querySelector(".credit-hours").value) || 0;
      ...
      if (isRetake) {
        ...
        const gradeDiff =
          this.gradeChart[newGrade] - this.gradeChart[previousGrade];
    ...
    const currentGPA =
      gpaCredits > 0 ? (currentPoints / gpaCredits).toFixed(2) : "0.00";
    const finalCGPA =
...
function toggleCustomInput() {
  var select = document.getElementById("waiverOrscholarship");
  var customInput = document.getElementById("customWaiver");
...
function calculateTutionFee() {
    let tuitionFee = parseFloat(document.getElementById("tuitionFee").value);
    // Selected input
    let waiverSelect = document.getElementById("waiverOrscholarship").value; 
    // For custom input
    let customWaiverInput = document.getElementById("customWaiver").value;
  
    let waiverPercentage =
      waiverSelect === "custom"
        ? parseFloat(customWaiverInput) || 0 
        : parseFloat(waiverSelect);
    ...
    if(waiverPercentage < 0){
        alert("Waiver percentage must be a positive number.");
        return;
    }
